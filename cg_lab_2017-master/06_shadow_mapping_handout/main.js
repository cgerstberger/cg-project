...
function drawScene(timeInMilliseconds) {
  ...
  console.log("timeInMilliseconds: " + timeInMilliseconds/100);
...
function moveCamera(curTimeInMilli, seconds, posX, posY, posZ, newPitch, newYaw){
  while(curTimeInMilli < seconds)
  {
...
function animate() {
  ...
  if (lastTime != 0) {
    ...
    if (backAndForth != 0) {
      xPos -= Math.sin(degToRad(yaw)) * backAndForth * elapsed;
      yPos -= Math.sin(degToRad(-pitch)) * backAndForth * elapsed;
      zPos -= Math.cos(degToRad(yaw)) * backAndForth * elapsed;
    }
