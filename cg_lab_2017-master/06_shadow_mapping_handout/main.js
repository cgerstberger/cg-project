...
function handleKeys() {
    ...
    console.log("backAndForth: " + backAndForth);
...
function drawScene(timeInMilliseconds) {
    ...
    //setup context and camera matrices
    const context = createSGContext(gl);
    context.projectionMatrix = mat4.perspective(mat4.create(), glm.deg2rad(30), gl.drawingBufferWidth / gl.drawingBufferHeight, 0.01, 500);
    //very primitive camera implementation
    let lookAtMatrix = mat4.lookAt(mat4.create(),
        [0 + camera.translation.x, 5 + camera.translation.y, -50 + camera.translation.z],
        [camera.translation.x + lookAtVec.translation.x, camera.translation.y + lookAtVec.translation.y, camera.translation.z + lookAtVec.translation.z],
        [0, 5, 0]);
    console.log("camera.x: " + camera.translation.x);
    console.log("camera.y: " + camera.translation.y);
    console.log("camera.z: " + camera.translation.z);
    console.log("lookAtVec.x: " + lookAtVec.translation.x);
    console.log("lookAtVec.y: " + lookAtVec.translation.y);
    console.log("lookAtVec.z: " + lookAtVec.translation.z);
    let mouseRotateMatrix = mat4.multiply(mat4.create(),
        glm.rotateX(-camera.rotation.y),
        glm.rotateY(-camera.rotation.x));
    mat4.rotate(mouseRotateMatrix, mouseRotateMatrix, degToRad(-yaw), [0, 1, 0]);
    context.viewMatrix = mat4.multiply(mat4.create(), mouseRotateMatrix, lookAtMatrix);

    mat4.rotate(context.viewMatrix, context.viewMatrix, degToRad(pitch), [1, 0, 0]);
    //mat4.rotate(context.viewMatrix, context.viewMatrix, degToRad(-yaw), [0, 1, 0]);
...
function animate() {
    ...
    if (lastTime != 0) {
        ...
        if (backAndForth != 0)
            camera.translation.z += backAndForth * elapsed;
        //if(leftAndRight != 0)
            //camera.translation.x += leftAndRight * elapsed;
        //if(upAndDown != 0)
            //camera.translation.y += upAndDown * elapsed;
...
function initInteraction(canvas) {
    ...
    /*document.addEventListener('keypress', function(event) {
        ...
        if (event.code === 'KeyW') {
            //var lookAtZ = lookAtVec.translation.z + 1;
