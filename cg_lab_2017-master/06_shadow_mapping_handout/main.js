...
function init(resources) {
  ...
  var floor4 = new AdvancedTextureSGNode(heightmapImage, new TransformationSGNode(glm.transform({
    translate: [-5, 0, -5]
  }), [new RenderSGNode(triangleStripModelRenderer(makeTriangleStripGrid(1, 5)))]));
...
function loadTexture() {
  ...
  heightmapImage.src = "../textures/terrain_heightmap.jpg";
}
...
function createSceneGraph(gl, resources) {
...

  {
    ...
    var floor2 = new MaterialSGNode(new TextureSGNode(renderTargetDepthTexture, 2, new RenderSGNode(makeBetterRect(1, 1))));
...
function render(timeInMilliseconds) {
  ...
  let lookAtMatrix = mat4.lookAt(mat4.create(), [0 + camera.translation.x, 5 + camera.translation.y, -30 + camera.translation.z], [0, 0, 0], [0, 1, 0]);
...
function makeTriangleStripGrid(size, divisions) {
  ...
  for (var i = 0; i <= divisions; ++i) {
    ...
    for (var j = 0; j <= divisions; ++j) {
      var rnd = Math.random();
      console.log("position[" + (i * segment_size) + "," + "rnd" + "," + (j * segment_size) + "]");
